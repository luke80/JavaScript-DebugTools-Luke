	...
	*/
	"prefix": function(prefix) {
		if(typeof prefix == "string")
			...
			this.o("Log Error: You can't set the prefix to a non-string. (You attempted to set it to a \""+typeof(prefix)+"\")");
	},
	/*
	...
	"htmlElement": function(id) {
		...
		if(typeof id == "string") {
			...
			//	If not, then we have some work to do...
				...
				this.o("Log: debug element HTML output element not found. Creating one and setting it.");
				...
				if($("body").length > 0) {
					...
					this.o("Log: putting off display of the output HTML element until ready event fires.");
					$(document).ready(function() {	//	On ready for the document, add the back-log of debug messages.
						if(Log._LOG_HTML_ELEMENT == null) {
							Log.o("Log: resetting log element.");
							Log._LOG_HTML_ELEMENT = $("<div class=\"debug-log\"></div>");
						}
						if($($("body")[0]).find("#"+Log._LOG_HTML_ELEMENT_ID).length == 0)	//	The output element may not be in the body already, if it's not - add it.
							$($("body")[0]).append(Log._LOG_HTML_ELEMENT);
						else
							$("#"+Log._LOG_HTML_ELEMENT_ID).append(Log._LOG_HTML_ELEMENT);	//	If the element is already found in the body, append the new content.
						//delete Log._LOG_HTML_ELEMENT_ID;	//	I played around with doing some garbage collection on this string. It seems unnecessary, so I took it out.
					});
				}
			}
		} else if(!id) {
		...
		*/
		...
		$(this._LOG_HTML_ELEMENT).bind("contextmenu", function() { $(Log._LOG_HTML_ELEMENT).css("display","none"); return false; } );
	...
	*/
			...
			if(typeof console.log != "undefined") {
				...
				if(typeof prefix != "string") {
					...
					if(this._LOG_CALLERLOOP_STACKTRACE_ON && this._LOG_CALLERLOOP_ON) {
						...
						if(prefix) {
							...
							this.stackTraceOff();
							this.o("Log: Notice; Stack trace cannot be performed, lack of caller identity.");
							this.stackTraceOn();
						}
				...
				//		In this last case, the output is an object of some sort. First we output the "prefix" data, then the object. Taking 2 lines.
					...
					try {
						...
						Log.o("log error: can't parse object (too large; \""+err.message+"\")");
					}
					if(this._TIMER_LOG) {
	...
	"html": function (msg) {
		if(this._LOG_HTML_ELEMENT != null) {
			...
			if(typeof outElement == "undefined") {
				console.log("Log Error: no output element yet.");
				outElement = $("<div></div>");
			}
			if(typeof jQuery != "undefined") {
	...
	"addShowHTMLLogButton": function() {
		if(!this._ADDED_BUTTON) {
			this._SHOW_BUTTON = $("<div title=\"Don't worry: this will not show up outside of a 'development' environment.\">Show Error Log</div>");
			...
			this._SHOW_BUTTON.bind("click",function() {
				Log.o("Log: Showing HTML log \""+"#"+Log._LOG_HTML_ELEMENT_ID+"\"");
				$("#"+Log._LOG_HTML_ELEMENT_ID).css("display","block");
				return false;
			});
			$(document).ready(function() {
				var parentElement = $($("#"+Log._LOG_HTML_ELEMENT_ID).parent());
				if(parentElement.length > 0) {
					$($("#"+Log._LOG_HTML_ELEMENT_ID).parent()).append(Log._SHOW_BUTTON);
					Log.o($($("#"+Log._LOG_HTML_ELEMENT_ID).parent()));
				} else {
					$($("body")[0]).append(Log._SHOW_BUTTON);
				}
				Log.o("Log: added HTML log show button.");
			});
		}
